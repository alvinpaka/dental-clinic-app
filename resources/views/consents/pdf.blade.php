...
<body>
  <div class="header">
    <div>
      ...
      <div class="muted">Patient: {{ $patient->name }} · Signed: {{ optional($consent->signed_at)->format('Y-m-d H:i') }}</div>
    </div>
    <div class="muted">#{{ $consent->id }}</div>
  </div>

  <div class="section box">
    {!! nl2br(e($consent->content_snapshot)) !!}
  </div>

  <div class="section box">
    <div><strong>Signed By:</strong> {{ $consent->signed_by_name }}</div>
    <div><strong>Signed At:</strong> {{ optional($consent->signed_at)->format('Y-m-d H:i') }}</div>
    @if($signature_path)
